...
$sql_arquivos->from(array('a' => 'db_sysarquivo'), array('a.*'), $config->schema)
                                ->join(array('b' => 'db_sysarqmod'), 'a.codarq = b.codarq',array('b.codmod'), $config->schema)
                                ->where('b.codmod = ?', $config->modulo);
// Arquivos
$arquivos = $db_ecidade->fetchAll($sql_arquivos);


$sql_campos->from(array('a' => 'db_syscampo'), array('a.*'), $config->schema)
...
foreach ($arquivos as $arquivo){
    ...
    file_put_contents('tmp/'.$nomeArquivo.'.php', $gerator->generate());
}
